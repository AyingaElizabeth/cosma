...
@admin.register(SiteSettings, site=cosma_admin)
class SiteSettingsAdmin(admin.ModelAdmin):
    fieldsets = (
        ('📍 Contact Information', {
            'fields': ('office_address', 'phone', 'email', 'office_hours'),
        }),
        ('🔗 Social Media Links', {
...
@admin.register(HeroSection, site=cosma_admin)
class HeroSectionAdmin(admin.ModelAdmin):
    list_display    = ('page_display', 'heading_preview', 'has_image', 'is_active', 'updated_at')
    list_editable   = ('is_active',)
    list_filter     = ('is_active',)
    search_fields   = ('page', 'heading', 'subheading')
    list_per_page   = 30
    ...
    fieldsets = (
        ...
        ('✏️ Content', {
            'description': 'Text shown over the hero image.',
            'fields': ('tag_icon', 'tag_text', 'heading', 'subheading'),
        }),
        ('🖼 Background Image', {
            'description': 'Upload an image OR paste a URL. Uploaded image takes priority.',
            'fields': ('image', 'image_preview', 'image_url', 'overlay_opacity'),
        }),
    ...
    @admin.display(description='Heading')
    def heading_preview(self, obj):
        return obj.heading[:60] if obj.heading else format_html('<em style="color:#aaa;">Not set</em>')
    ...
    def image_preview(self, obj):
        url = obj.bg_image_url
        if url:
            return format_html('<img src="{}" style="max-width:400px;border-radius:10px;max-height:200px;object-fit:cover;"/>', url)
        return 'No image set'


# ─── Home Slides ─────────────────────────────────────────────────────────────

@admin.register(HomeSlide, site=cosma_admin)
class HomeSlideAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('⚙ Settings', {'fields': ('caption', 'order', 'is_active')}),
    )
    ...
    def thumb(self, obj):
        url = obj.bg_url
        if url:
            return format_html('<img src="{}" style="width:72px;height:48px;border-radius:6px;object-fit:cover;"/>', url)
        return '—'
    ...
    def image_preview(self, obj):
        url = obj.bg_url
        if url:
            return format_html('<img src="{}" style="max-width:400px;border-radius:10px;"/>', url)
        return 'No image set'
...
@admin.register(ImpactItem, site=cosma_admin)
class ImpactItemAdmin(admin.ModelAdmin):
    list_display  = ('emoji', 'figure', 'desc', 'order')
    list_editable = ('order',)
    ordering      = ('order',)
...
class MissionVisionPurposeAdmin(admin.ModelAdmin):
    fieldsets = (
        ('🎯 Organisation Direction', {
            'fields': ('mission', 'vision', 'purpose', 'goal'),
        }),
        ('🕒 Audit', {'fields': ('updated_at',), 'classes': ('collapse',)}),
    )
    ...
    def has_add_permission(self, request):
        return not MissionVisionPurpose.objects.exists()


# ─── Timeline Events ─────────────────────────────────────────────────────────

@admin.register(TimelineEvent, site=cosma_admin)
class TimelineEventAdmin(admin.ModelAdmin):
    list_display    = ('year', 'title', 'stat', 'has_image', 'order')
    list_editable   = ('order',)
    ...
    fieldsets = (
        ('📅 Event', {'fields': ('year', 'icon', 'title', 'desc', 'stat', 'order')}),
        ('🖼 Image', {'fields': ('image', 'image_preview', 'image_url')}),
    )
    ...
    def image_preview(self, obj):
        url = obj.image_src
        if url:
            return format_html('<img src="{}" style="max-width:300px;border-radius:10px;"/>', url)
        return 'No image'


# ─── Org Goals ───────────────────────────────────────────────────────────────

@admin.register(OrgGoal, site=cosma_admin)
class OrgGoalAdmin(admin.ModelAdmin):
    list_display  = ('title', 'progress_bar', 'order')
    list_editable = ('order',)
    ordering      = ('order',)
    ...
    def progress_bar(self, obj):
        return format_html(
            '<div style="background:#eee;border-radius:4px;width:150px;height:8px;">'
            '<div style="background:#3BAD6E;width:{}%;height:100%;border-radius:4px;"></div></div>'
            '<small style="color:#666;">{} %</small>',
            obj.progress, obj.progress)


# ─── Hero Stats (Our Story) ───────────────────────────────────────────────────

@admin.register(HeroStat, site=cosma_admin)
class HeroStatAdmin(admin.ModelAdmin):
    list_display  = ('label', 'raw', 'suffix', 'order')
    list_editable = ('order',)
    ordering      = ('order',)
...
@admin.register(VocationalCourse, site=cosma_admin)
class VocationalCourseAdmin(admin.ModelAdmin):
    list_display  = ('title', 'duration', 'icon', 'is_active', 'order')
...
@admin.register(AgriApproachStep, site=cosma_admin)
class AgriApproachStepAdmin(admin.ModelAdmin):
...
@admin.register(SponsorStep, site=cosma_admin)
class SponsorStepAdmin(admin.ModelAdmin):
    list_display  = ('title', 'order')
    list_editable = ('order',)
    ordering      = ('order',)
...
@admin.register(PartnershipType, site=cosma_admin)
class PartnershipTypeAdmin(admin.ModelAdmin):
...
@admin.register(VolunteerRole, site=cosma_admin)
class VolunteerRoleAdmin(admin.ModelAdmin):
    list_display  = ('title', 'commitment', 'is_active', 'order')
    list_editable = ('is_active', 'order')
    ordering      = ('order',)
...
@admin.register(ImpactBigStat, site=cosma_admin)
class ImpactBigStatAdmin(admin.ModelAdmin):
...
@admin.register(ImpactProgrammeStat, site=cosma_admin)
class ImpactProgrammeStatAdmin(admin.ModelAdmin):
    list_display  = ('label', 'value', 'progress_bar', 'order')
    list_editable = ('order',)
    ordering      = ('order',)
...
class ImpactStoryAdmin(admin.ModelAdmin):
    ...
    def tag_badge(self, obj):
        c = {'Education': '#3BAD6E', 'Health': '#8E0005', 'Agriculture': '#E67E22',
             'Livelihoods': '#2980B9', 'Clean Water': '#16A085', 'Empowerment': '#8E44AD'}.get(obj.tag, '#666')
    ...
    def make_featured(self, req, qs):
        ...
        self.message_user(req, f'{n} stories marked as featured.', messages.SUCCESS)

    @admin.action(description='✖ Remove from Featured')
    def remove_featured(self, req, qs):
        n = qs.update(is_featured=False)
        self.message_user(req, f'{n} stories hidden.', messages.WARNING)

    @admin.action(description='⬇ Export to CSV')
    def export_csv_action(self, req, qs):
        return export_csv(qs, 'impact_stories.csv', ['headline', 'name', 'location', 'programme', 'tag', 'author', 'is_featured', 'created_at'])
...
class PartnerAdmin(admin.ModelAdmin):
    ...
    def logo_thumb(self, obj):
        ...
        return format_html('<img src="{}" style="height:34px;max-width:90px;object-fit:contain;"/>', src) if src else '—'

    @admin.display(description='Website')
    def website_link(self, obj):
        return format_html('<a href="{}" target="_blank" style="color:#3BAD6E;">🔗 Visit</a>', obj.website) if obj.website else '—'
...
class DonationLeadAdmin(admin.ModelAdmin):
    ...
    @admin.action(description='⬇ Export to CSV')
    def export_csv_action(self, req, qs):
        return export_csv(qs, 'donations.csv', ['name', 'email', 'amount', 'message', 'created_at'])
...
class ContactMessageAdmin(admin.ModelAdmin):
    ...
    def status_dot(self, obj):
        c, t = ('#8E0005', 'Unread') if not obj.is_read else ('#ddd', 'Read')
    ...
    def email_link(self, obj):
        return format_html('<a href="mailto:{}" style="color:#3BAD6E;">{}</a>', obj.email, obj.email)

    @admin.display(description='Reply')
    def quick_reply(self, obj):
        return format_html('<a href="mailto:{}?subject=Re: {}" style="background:#3BAD6E;color:#fff;padding:5px 12px;border-radius:6px;text-decoration:none;font-size:.8rem;">✉ Reply</a>', obj.email, obj.subject)

    @admin.action(description='✔ Mark as Read')
    def mark_read(self, req, qs):
        n = qs.update(is_read=True)
        self.message_user(req, f'{n} messages marked as read.', messages.SUCCESS)

    @admin.action(description='✖ Mark as Unread')
    def mark_unread(self, req, qs):
        n = qs.update(is_read=False)
        self.message_user(req, f'{n} messages marked as unread.', messages.INFO)

    @admin.action(description='⬇ Export to CSV')
    def export_csv_action(self, req, qs):
        return export_csv(qs, 'messages.csv', ['name', 'email', 'subject', 'message', 'is_read', 'created_at'])
...
class ResourceAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('📄 Publication Details', {'fields': ('title', 'description', 'resource_type', 'year', 'pages')}),
        ('📁 File Upload', {'fields': ('file', 'file_preview_link', 'ext_ro', 'size_ro', 'cover_image')}),
        ('⚙ Visibility & Ordering', {'fields': ('is_published', 'is_featured', 'order')}),
        ('🕒 Audit', {'fields': ('created_at', 'updated_at'), 'classes': ('collapse',)}),
    )
    ...
    def thumb_preview(self, obj):
        if obj.pk and obj.cover_image:
            return format_html('<img src="{}" style="width:42px;height:42px;border-radius:8px;object-fit:cover;"/>', obj.cover_image.url)
    ...
    def type_badge(self, obj):
        pal = {'annual_report': ('#8E0005', '#FEE2E2'), 'research': ('#1D4ED8', '#DBEAFE'),
               'policy_brief': ('#6D28D9', '#EDE9FE'), 'programme_update': ('#065F46', '#D1FAE5'),
               'media_kit': ('#92400E', '#FEF3C7'), 'other': ('#374151', '#F3F4F6')}
        fg, bg = pal.get(obj.resource_type, ('#374151', '#F3F4F6'))
        return format_html('<span style="background:{};color:{};padding:3px 10px;border-radius:50px;font-size:.72rem;font-weight:700;">{}</span>', bg, fg, obj.get_resource_type_display())
    ...
    def featured_col(self, obj):
        return format_html('<span style="color:#D97706;font-weight:700;">⭐ Yes</span>') if obj.is_featured else '—'
    ...
    def file_preview_link(self, obj):
        if obj.pk and obj.file:
            return format_html('<a href="{}" target="_blank" style="color:#3BAD6E;font-weight:600;">🔗 Open</a>', obj.file.url)
        return '—'
    ...
    @admin.action(description='⭐ Set as Featured report')
    def action_feature(self, req, qs):
        Resource.objects.update(is_featured=False)
        n = qs.update(is_featured=True)
        self.message_user(req, f'{n} resource(s) set as featured.', messages.SUCCESS)
    ...
    @admin.action(description='⬇ Export to CSV')
    def export_csv_action(self, req, qs):
        return export_csv(qs, 'resources.csv', ['title', 'resource_type', 'year', 'is_published', 'is_featured', 'created_at'])
...
class TestimonialAdmin(admin.ModelAdmin):
    list_display        = ('photo_thumb', 'name', 'role_badge', 'organisation', 'stars_col', 'approved_col', 'featured_col', 'order', 'created_at')
    list_editable       = ('order',)
    ...
    fieldsets = (
        ('👤 Person', {'fields': ('name', 'role', 'organisation', 'photo', 'photo_preview')}),
        ('💬 Testimonial', {'fields': ('body', 'rating')}),
        ('⚙ Visibility', {'fields': ('is_approved', 'is_featured', 'order')}),
    ...
    def photo_thumb(self, obj):
        ...
        return format_html('<div style="width:40px;height:40px;border-radius:50%;background:linear-gradient(135deg,#8E0005,#c0392b);display:flex;align-items:center;justify-content:center;color:#fff;font-size:.75rem;font-weight:700;">{}</div>', initials)

    @admin.display(description='Role')
    def role_badge(self, obj):
        colours = {'donor': '#2563EB,#DBEAFE', 'volunteer': '#059669,#D1FAE5',
                   'partner': '#7C3AED,#EDE9FE', 'beneficiary': '#3BAD6E,#D1FAE5',
                   'community': '#D97706,#FEF3C7', 'other': '#6B7280,#F3F4F6'}
    ...
    def stars_col(self, obj):
        return format_html('{}{}',
            format_html('<span style="color:#F59E0B;">★</span>') * obj.rating,
            format_html('<span style="color:#E5E7EB;">★</span>') * (5 - obj.rating))

    @admin.display(description='Approved')
    def approved_col(self, obj):
        return format_html('<span style="color:#059669;font-weight:600;">✔ Live</span>') if obj.is_approved else format_html('<span style="color:#DC2626;">✖ Pending</span>')

    @admin.display(description='Featured')
    def featured_col(self, obj):
        return format_html('<span style="color:#D97706;">⭐</span>') if obj.is_featured else '—'
    ...
    def approve(self, req, qs):
        ...
        self.message_user(req, f'{n} approved.', messages.SUCCESS)

    @admin.action(description='✖ Unapprove')
    def unapprove(self, req, qs):
        n = qs.update(is_approved=False)
        self.message_user(req, f'{n} hidden.', messages.WARNING)

    @admin.action(description='⭐ Mark as Featured')
    def make_featured(self, req, qs):
        n = qs.update(is_featured=True)
        self.message_user(req, f'{n} featured.', messages.SUCCESS)

    @admin.action(description='Remove from Featured')
    def remove_featured(self, req, qs):
        n = qs.update(is_featured=False)
        self.message_user(req, f'{n} removed from featured.', messages.WARNING)
...
class NewsPostAdmin(admin.ModelAdmin):
    list_display        = ('cover_thumb', 'title', 'category_badge', 'author', 'read_time_col', 'published_col', 'featured_col', 'published_at')
    ...
    fieldsets = (
        ('📰 Article Content', {'fields': ('title', 'slug', 'slug_note', 'category', 'author', 'summary', 'body')}),
    ...
    def read_time_col(self, obj): return obj.read_time

    @admin.display(description='Status')
    def published_col(self, obj):
        return format_html('<span style="color:#059669;font-weight:600;">✔ Live</span>') if obj.is_published else format_html('<span style="color:#DC2626;">✖ Draft</span>')

    @admin.display(description='Featured')
    def featured_col(self, obj):
        return format_html('<span style="color:#D97706;font-weight:700;">⭐</span>') if obj.is_featured else '—'
    ...
    def read_time_ro(self, obj): return obj.read_time if obj.pk else '—'

    @admin.display(description='')
    def slug_note(self, obj):
        return format_html('<span style="color:#6B7280;font-size:.82rem;">Auto-generated from title on first save.</span>')
    ...
    @admin.action(description='✖ Unpublish (draft)')
